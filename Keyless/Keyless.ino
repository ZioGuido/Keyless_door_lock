...
void StartMotor()
{
  ...
  case opOpen:
    ...
    // Turn until the end of coarse is found, this will beat the spring's force and keep the door open. 
  ...
  case opLock:
    ...
    if (digitalRead(PIN_J_NO_RELEASE) == LOW) RequestedPulses += configuration.len_release;
    break;
...
void loop() 
{
  ...
  else
  {
    ...
    // Lock if door is close, D&N is on and the motor is in released position
    if (DayAndNight == HIGH && DoorStatus == LOW && CurrentMotorPosition == posReleased)
    {
